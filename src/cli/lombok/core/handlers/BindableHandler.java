 ...
 */
public abstract class BindableHandler<TYPE_TYPE extends IType<?, FIELD_TYPE, ?, ?, ?, ?>, FIELD_TYPE extends IField<?, ?, ?>, LOMBOK_NODE_TYPE extends LombokNode<?, LOMBOK_NODE_TYPE, ?>, SOURCE_TYPE> extends AbstractHandler<TYPE_TYPE> implements BindableConstants {
    ...
    public BindableHandler(LOMBOK_NODE_TYPE annotationNode, SOURCE_TYPE ast) {
    ...
    public void handle() {
        ...
        if (mayBeField.getKind() == AST.Kind.FIELD) {
            ...
            annotationNode.addError(canBeUsedOnClassAndFieldOnly(Bindable.class));
            return;
        }
    ...
    protected void apply(TYPE_TYPE type, List<FIELD_TYPE> fields) {
        if (!fields.isEmpty()) {
            if (!hasAllPropertyChangeMethods(type)) {
                addInterface(type, GRIFFON_CORE_OBSERVABLE_TYPE);
