...
import lombok.core.VetoableConstants;
 ...
 */
public abstract class VetoableHandler<TYPE_TYPE extends IType<?, FIELD_TYPE, ?, ?, ?, ?>, FIELD_TYPE extends IField<?, ?, ?>, LOMBOK_NODE_TYPE extends LombokNode<?, LOMBOK_NODE_TYPE, ?>, SOURCE_TYPE> extends BindableHandler<TYPE_TYPE, FIELD_TYPE, LOMBOK_NODE_TYPE, SOURCE_TYPE> implements VetoableConstants {
    ...
    protected void apply(TYPE_TYPE type, List<FIELD_TYPE> fields) {
        if (!fields.isEmpty()) {
            if (!hasAllVetoableChangeMethods(type)) {
                addInterface(type, GRIFFON_CORE_VETOABLE_TYPE);
